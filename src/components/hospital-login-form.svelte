...
<main class="outline outline-2 outline-black p-4 rounded-lg shadow-md flex flex-col w-80">
    ...
    {#if selection === "login"}
        <h3>Hospital log in to Man.man</h3>
        <form method="POST" action="/hospital/api/login">
            <label for="email">
                Enter the hospital email:
            </label>
    ...
    {:else}
        ...
        <form method="POST" action="/hospital/api/signup">
            <label for="hospital-name">
                Enter your hospital's name:
            </label>
            <input type="text" name="hospital-name" id="hospital-name" required>
            <label for="email">
                Enter your hospital's email:
            </label>
            <input type="email" name="email" id="email" required>
            <label for="password">
                Enter a password to access and manage all doctors:
            </label>
            <input type="password" name="password" id="password" required bind:value={password}>
            <label for="repeat-password">
                Repeat the password: <br> {#if password !== repeat_password_alert && repeat_password_alert !== null} <span class="text-red-600 font-bold" transition:fade>Please ensure passwords match</span> {/if}
            </label>
